...
class Tokenizer:
    ...
    def encode(self, text: str) -> list[int]:
        ...
        if special_tokens:
            special_pattern = "|".join(re.escape(k) for k in sorted(special_tokens, key=len, reverse=True))
                ...
                for match in re.finditer(PAT, chunk):
                    ...
                    while can_merge:
                        ...
                        for bytes_pair in zip(word_bytes, word_bytes[1:]):
                            ...
                            merge_indexs.append(merges_dict.get(bytes_pair, len(merges) + 1))
                        # Merge the min indexs
                        min_merge_index = min(merge_indexs)
                        for i in range(len(word_bytes)-1):
                            if merge_indexs[i] == min_merge_index and merge_indexs[i] != len(merges) + 1:
                                word_bytes = word_bytes[:i] + [word_bytes[i]+word_bytes[i+1]] + word_bytes[i+2:]
                                can_merge = True
                                break
