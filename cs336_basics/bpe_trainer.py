    ...
    def train_byte(
        ...
        **kwargs,
        ...
        for pretoken, count in pretoken_counts.items():
            byte_pretoken = pretoken.encode("utf-8")
            byte_pretoken_counts[byte_pretoken] = count

        # 3. Merge
        for _ in tqdm(range(vocab_size - len(vocab)), desc="Training BPE"):
            byte_freq = Counter()
            for byte_pretoken, count in byte_pretoken_counts.items():
                for index in range(len(byte_pretoken) - 1):
                    byte_pair = (
                        chr(byte_pretoken[index]).encode("utf-8"),
                        chr(byte_pretoken[index + 1]).encode("utf-8"),
                    )
            ...
            for old_pretoken, count in byte_pretoken_counts.items():
                new_pretoken = old_pretoken
                i = 0
                while i < len(new_pretoken) - 1:
                    if (
                        chr(new_pretoken[i]).encode("utf-8"),
                        chr(new_pretoken[i + 1]).encode("utf-8"),
                    ) == new_token:
                        new_pretoken = (
                            new_pretoken[:i]
                            + new_token[0]
